  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={() => { setIsCreating(true); setEditing({ title: "", company: "", isCurrent: false, sortOrder: 0 }); }}
      ...
      {(editing || isCreating) && (
        <div className="portfolio-card p-6 mb-6">
          <h2 className="text-white font-semibold text-lg mb-4">{isCreating ? "Create" : "Edit"} Employment</h2>
          ...
          <div key={item.id} className="portfolio-card p-4 flex items-center justify-between">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {item.isCurrent && (
                  <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: "rgba(71,184,255,0.15)", color: "#47b8ff" }}>Current</span>
                )}
              </div>
              <p className="text-white/60 text-xs mt-0.5">{item.company}{item.duration ? ` — ${item.duration}` : ""}</p>
            </div>
